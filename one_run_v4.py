# -*- coding: utf-8 -*-
"""
one_run.py — Tek dosyalık pipeline
...
# Hesaplama kapasitesi artırımı için ayarlar
N_THREADS = min(os.cpu_count() or 16, 32)  # Maksimum thread sayısını artır
MEMORY_LIMIT_GB = 26  # 26GB RAM sınırı
...
def _callbacks():
    return [lgb.early_stopping(stopping_rounds=600, verbose=True),  # Daha uzun sabır
            lgb.log_evaluation(period=200)]  # Daha az log
...
def train_ltr_models(tr: pd.DataFrame, va: pd.DataFrame, feat_cols: list):
    # click - daha agresif parametreler
    ...
    params_click = {
        ...
        "eval_at": [5, 10, 20, 100],
        "boosting": "gbdt",
        "learning_rate": 0.055,  # Biraz azaltıldı, iterasyonlar artırıldığı için
        "num_leaves": 127,       # Artırıldı
        "max_depth": 12,         # Artırıldı
        "min_data_in_leaf": 30,  # Azaltıldı (daha fazla detay)
        "feature_fraction": 0.85, # Azaltıldı (regularization)
        "bagging_fraction": 0.75,  # Azaltıldı (regularization)
        "bagging_freq": 1,
        "lambda_l1": 0.1,        # L1 regularization eklendi
        "lambda_l2": 0.1,        # L2 regularization eklendi
        "verbose": -1,
        "num_threads": N_THREADS,
    }
    m_click = lgb.train(params_click, dtr, num_boost_round=8000,  # Artırıldı
                        valid_sets=[dtr, dva], valid_names=["train", "valid"],
                        callbacks=_callbacks())
    
    # order - daha agresif parametreler
    ...
    params_order = {
        ...
        "eval_at": [5, 10, 20, 100],
        "boosting": "gbdt",
        "learning_rate": 0.055,  # Biraz azaltıldı
        "num_leaves": 127,       # Artırıldı
        "max_depth": 12,         # Artırıldı
        "min_data_in_leaf": 30,  # Azaltıldı
        "feature_fraction": 0.85, # Azaltıldı (regularization)
        "bagging_fraction": 0.75,  # Azaltıldı (regularization)
        "bagging_freq": 1,
        "lambda_l1": 0.1,        # L1 regularization eklendi
    ...
    m_order = lgb.train(params_order, dtr, num_boost_round=8000,  # Artırıldı
                        valid_sets=[dtr, dva], valid_names=["train", "valid"],
                        callbacks=_callbacks())
...
def run_train_xgb():
    ...
    ranker = XGBRanker(
        ...
        reg_alpha=0.1,          # L1 regularization eklendi
        reg_lambda=1.5,         # L2 artırıldı
        random_state=42,
...
def run_train_cat():
    ...
    model = CatBoostRanker(
        ...
        iterations=6000,        # Artırıldı
        learning_rate=0.04,     # Azaltıldı (daha fazla iterasyon için)
        depth=10,               # Artırıldı
        l2_leaf_reg=5.0,        # Artırıldı (regularization)
        random_strength=1.5,    # Artırıldı
        bootstrap_type="Bayesian",
        bagging_temperature=0.8, # Eklendi
        od_type="Iter",
        od_wait=600,            # Artırıldı (daha uzun sabır)
        verbose=100,
        random_seed=42,
        thread_count=N_THREADS,
        task_type=("GPU" if gpu_available() else "CPU"),
        border_count=128,       # Feature discretization kalitesi artırıldı
        max_ctr_complexity=6,   # Categorical feature interaction artırıldı
    )
...
if __name__ == "__main__":
    ...
    if args.baseline_timeaware:
        ...
        print("✨ Optimize Edilmiş Örnekler (6dk+ çalışma süresi için):\n"
              "🎯 Ana modellerle eğitim ve infer:\n"
              "  python one_run_v4.py --train_ltr --alpha 0.80\n"
              "  python one_run_v4.py --infer_ltr --alpha 0.80 --out outputs/sub_ltr_optimized.csv\n"
              "\n🚀 XGBoost & CatBoost (artırılmış kapasiteli):\n"
              "  python one_run_v4.py --train_xgb\n"
              "  python one_run_v4.py --infer_xgb --out outputs/sub_xgb_optimized.csv\n"
              "  python one_run_v4.py --train_cat\n"
              "  python one_run_v4.py --infer_cat --out outputs/sub_cat_optimized.csv\n"
              "\n🔥 En İyi Ensemble (tüm modelleri birleştir):\n"
              "  python one_run_v4.py --infer_ensemble --alpha 0.82 --w_ltr 0.45 --w_xgb 0.25 --w_cb 0.25 --w_ta 0.05 --out outputs/sub_ensemble_best.csv\n"
              "\n📊 Performans testi:\n"
              "  python one_run_v4.py --offline_eval --alpha 0.82 --w_ltr 0.45 --w_xgb 0.25 --w_cb 0.25 --w_ta 0.05\n"
              "\n💡 Gelişmiş Baseline (hızlı başlangıç):\n"
              "  python one_run_v4.py --baseline_timeaware --out outputs/ta_optimized.csv\n"
              "\n📈 Artırılmış Özellikler:\n"
              "• 5 farklı zaman penceresi (3,7,14,30,60 gün)\n" 
              "• GPU desteği (varsa otomatik kullanım)\n"
              "• 26GB RAM kullanım limiti\n"
              "• Artırılmış model karmaşıklığı\n"
              "• Yeni cross-feature'lar ve trend analizi\n"
              "• Gelişmiş oturum-içi özellikler")
