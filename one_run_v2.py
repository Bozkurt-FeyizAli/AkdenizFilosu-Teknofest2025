...
def gpu_available() -> bool:
    """Basit GPU algılama: CUDA görünürse True. Kütüphane GPU ile derlenmemişse fallback olur."""
    dev = os.environ.get("CUDA_VISIBLE_DEVICES", None)
    return dev is not None and dev != "" and dev != "-1"
...
def run_infer_xgb(out_path: str):
    ...
    model = XGBRanker()
    # GPU varsa tahminciyi GPU'ya almayı dene (uyumsuzsa XGBoost CPU'ya düşer)
    try:
        if gpu_available():
...
def run_infer_cat(out_path: str):
    ...
    model = CatBoostRanker()
    try:
        model.set_params(thread_count=N_THREADS)
    except Exception:
        pass
