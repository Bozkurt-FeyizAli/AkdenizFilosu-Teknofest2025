...
def generate_features_v3(data_dir: Path, is_train: bool):
    """
    Polars kullanarak train veya test seti için TÜM özellikleri üretir ve tek bir parquet dosyası olarak kaydeder.
    Versiyon 3: Kullanıcı-kategori etkileşimleri ve kategoriye göre normalize fiyat özellikleri eklendi.
    """
    
    print(f"\n[{'TRAIN' if is_train else 'TEST'}] için Gelişmiş Özellik Üretimi (v3) başlıyor...")
    
    # ---------------------------------
    # 1. Gerekli Tüm Verileri Lazy Oku
    # ---------------------------------
    if is_train:
        # ÖNEMLİ: Yeni özellikler için sessions_lazy'e content_meta'yı en başta ekleyelim
        sessions_lazy = pl.scan_parquet(data_dir / "train_sessions.parquet")
        content_meta_lazy_for_join = pl.scan_parquet(data_dir / "content" / "metadata.parquet").select("content_id_hashed", "leaf_category_name")
        sessions_lazy = sessions_lazy.join(content_meta_lazy_for_join, on="content_id_hashed", how="left")
    else:
    ...
    # ---------------------------------
    # ### YENİ ÖZELLİK BLOĞU (v3) ###
    # ---------------------------------
    print("   - Yeni v3 özellikleri (etkileşim, kategori-fiyat) ekleniyor...")
    
    # Fikir 1: Kullanıcının her bir leaf_category'deki etkileşim sayıları
    user_cat_interaction_feats = sessions_lazy.group_by("user_id_hashed", "leaf_category_name").agg(
        pl.count().alias("user_cat_interaction_count")
    )
    main_df_lazy = main_df_lazy.join(
        user_cat_interaction_feats, on=["user_id_hashed", "leaf_category_name"], how="left"
    )

    # Fikir 2: Her bir kategorinin ortalama fiyatını hesapla
    category_price_feats = main_df_lazy.group_by("leaf_category_name").agg(
        pl.col("discounted_price").mean().alias("mean_price_for_category")
    )
    main_df_lazy = main_df_lazy.join(category_price_feats, on="leaf_category_name", how="left")
    ...
    final_lazy = main_df_lazy.with_columns(
        ...
        discount_rate = (1 - pl.col("original_price") / pl.col("original_price")).fill_nan(0),
        search_term_in_cv_tags = pl.col("cv_tags").str.contains(pl.col("search_term_normalized"), literal=True).cast(pl.Int8).fill_null(0),

        # ### YENİ ÖZELLİK HESAPLAMASI (v3) ###
        price_vs_cat_mean = pl.col("discounted_price") / (pl.col("mean_price_for_category") + 1e-6),

    ).with_columns(
        # Oturum İçi (In-Session) Özellikler
    ...
    # Çıktı dosyasının adını da güncelleyelim ki eski sonuçların üzerine yazılmasın
    output_path = data_dir / f"{'train' if is_train else 'test'}_features_v3.parquet"
