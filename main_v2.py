...
from build_features_polars_v2 import generate_features_v2
...
def run_training(args):
    ...
    train_feature_path = DATA / "train_features_v2.parquet"
    test_feature_path = DATA / "test_features_v2.parquet"

    if not train_feature_path.exists() or not test_feature_path.exists() or args.force_rebuild:
        print("Gelişmiş özellik dosyaları (v2) bulunamadı veya yeniden oluşturma zorlandı.")
        generate_features_v2(DATA, is_train=True)
        generate_features_v2(DATA, is_train=False)
    else:
        print("Mevcut gelişmiş özellik dosyaları (v2) kullanılacak.")
