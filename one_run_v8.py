# -*- coding: utf-8 -*-
"""
one_run_v8.py — Tek dosyalık pipeline
...
# ====================== YENİ: METİN BENZERLİK ÖZELLİĞİ FONKSİYONU ======================
def generate_text_similarity_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Sentence-Transformers kullanarak arama terimi ve cv_tags arasında anlamsal
    benzerlik skoru üreten fonksiyon.
    Embeddings'leri sonraki kullanımlar için kaydeder.
    """
    with timer("Text Similarity Feature Generation"):
        try:
            ...
            print("[WARN] sentence-transformers kütüphanesi bulunamadı. Bu özellik atlanıyor.")
            return df

        # Model ve embedding'lerin kaydedileceği yollar
        ...
        # --- Arama Terimi Embedding'leri ---
        if os.path.exists(term_emb_path):
            print("Mevcut arama terimi embedding'leri yükleniyor.")
            data = np.load(term_emb_path, allow_pickle=True)
            unique_terms = data['keys']
            term_embeddings = data['vectors']
        else:
        ...
        term_to_emb = {term: emb for term, emb in zip(unique_terms, term_embeddings)}

        # --- Ürün (CV Tags) Embedding'leri ---
        if os.path.exists(content_emb_path):
            print("Mevcut ürün (cv_tags) embedding'leri yükleniyor.")
            data = np.load(content_emb_path, allow_pickle=True)
            unique_contents = data['keys']
            content_embeddings = data['vectors']
        else:
            print("Ürün (cv_tags) embedding'leri oluşturuluyor...")
            meta = pd.read_parquet(os.path.join(DATA_DIR, "content/metadata.parquet"), columns=["content_id_hashed", "cv_tags"])
            meta = meta.dropna(subset=["content_id_hashed", "cv_tags"])
            meta = meta.drop_duplicates(subset=["content_id_hashed"]).set_index("content_id_hashed")
            
            unique_contents = meta.index.to_numpy()
            tags_to_encode = meta.loc[unique_contents, "cv_tags"].tolist()
        ...
        # Eksik embedding'ler için default vektör (sıfır vektörü)
        ...
        numerator = np.sum(term_vecs_np * content_vecs_np, axis=1)
        denominator = np.linalg.norm(term_vecs_np, axis=1) * np.linalg.norm(content_vecs_np, axis=1)
        
        # Sıfıra bölme hatasını engelle
        similarity_scores = np.divide(numerator, denominator, out=np.zeros_like(numerator, dtype=float), where=denominator!=0)
        
        df["term_cv_similarity"] = similarity_scores.astype("float32")
        
    return df
# =====================================================================================

# ====================== YENİ: KULLANICI PROFİLİ ÖZELLİKLERİ ======================
def generate_user_profile_features(df: pd.DataFrame) -> pd.DataFrame:
    with timer("User Profile Feature Generation"):
        ...
        if os.path.exists(profile_path):
            print("Mevcut kullanıcı profilleri yükleniyor.")
            ...
            price = pd.read_parquet(os.path.join(DATA_DIR, "content/price_rate_review_data.parquet"), 
                                    columns=["content_id_hashed", "selling_price"])

            # Sadece sipariş olan etkileşimleri al
            ordered_sessions = sessions[sessions['ordered'] == 1].dropna(subset=['user_id_hashed'])

            # Fiyat ve kategori bilgilerini birleştir
            ordered_sessions = ordered_sessions.merge(meta, on="content_id_hashed", how="left")

            # Her ürünün son bilinen fiyatını al
            last_price = price.sort_values('update_date').drop_duplicates('content_id_hashed', keep='last')
            ordered_sessions = ordered_sessions.merge(last_price, on="content_id_hashed", how="left")

            # Kullanıcının favori kategorisini bul (en çok sipariş verdiği)
            fav_cat = ordered_sessions.groupby(["user_id_hashed", "leaf_category_name"]).size().reset_index(name="counts")
            fav_cat = fav_cat.sort_values("counts", ascending=False).drop_duplicates("user_id_hashed").rename(columns={"leaf_category_name": "user_fav_category"})

            # Kullanıcının ortalama harcama fiyatını bul
            avg_price = ordered_sessions.groupby("user_id_hashed")["selling_price"].mean().reset_index().rename(columns={"selling_price": "user_avg_order_price"})

            # Profilleri birleştir
            user_profiles = fav_cat[["user_id_hashed", "user_fav_category"]].merge(avg_price, on="user_id_hashed", how="outer")
            user_profiles.to_parquet(profile_path)

        # Ana DataFrame'e ürün kategori bilgisini ekle
        if 'leaf_category_name' not in df.columns:
             meta_cats = pd.read_parquet(os.path.join(DATA_DIR, "content/metadata.parquet"), columns=["content_id_hashed", "leaf_category_name"])
             df = df.merge(meta_cats, on="content_id_hashed", how="left")

        # Ana DataFrame'e kullanıcı profillerini ekle
        ...
        # Ürün fiyatının kullanıcının ortalama harcamasına oranı
        df["price_vs_user_avg"] = (df["selling_price"] / (df["user_avg_order_price"] + 1e-6)).fillna(1.0).astype("float32")
