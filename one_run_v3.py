...
def score_timeaware_baseline(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    # Click propensity: term×content CTR + user search behavior boosts
    out["pred_click"] = (
        0.60 * out.get("tc_ctr_7d", 0).fillna(0)
        + 0.25 * out.get("tc_ctr_30d", 0).fillna(0)
        + 0.08 * out.get("content_search_ctr_7d", 0).fillna(0)
        + 0.04 * out.get("user_search_ctr_30d", 0).fillna(0)
        + 0.03 * out.get("user_term_ctr_30d", 0).fillna(0)
    )

    # Order propensity: recent order-rate dominates + personalized order/cart history + promos/quality
    out["pred_order"] = (
        0.78 * out.get("order_rate_7d", 0).fillna(0)
        + 0.22 * out.get("order_rate_30d", 0).fillna(0)
    )
    out["pred_order"] += 0.05 * out.get("discount_pct", 0).clip(lower=0).fillna(0)
    out["pred_order"] += 0.02 * (out.get("rating_avg", 0).fillna(0) / 5.0)
    # personalized historical signals (safe if missing)
    out["pred_order"] += 0.05 * out.get("user_content_fashion_order_rate_30d", 0).fillna(0)
    out["pred_order"] += 0.03 * out.get("user_sw_order_rate_30d", 0).fillna(0)
    out["pred_order"] += 0.02 * out.get("user_sw_cart_rate_30d", 0).fillna(0)

    # final blend favors order (competition weights prioritize orders)
...
def run_infer_ensemble(out_path: str,
    ...
    extra_bonus_cols = [c for c in ["discount_rank_sess","cheap_rank_sess","rating_avg_rank_sess"] if c in feats_te.columns]
    sel_cols = key + ["seen_before"] + missing_cols + extra_bonus_cols
    ...
    df["repeat_bonus"] = (
        ...
        0.10 * (df["added_to_cart_before_item_sess"].fillna(0) > 0).astype("float32") +
        0.06 * (df["added_to_fav_before_item_sess"].fillna(0) > 0).astype("float32") +
        0.04 * (df["clicked_before_item_sess"].fillna(0) > 0).astype("float32")
    ).astype("float32")

    # fiyat/indirim/kaliteye dayalı nazik tie-breaker bonusları
    session_bonus = np.float32(0.0)
    if "discount_rank_sess" in df.columns:
        session_bonus = session_bonus + 0.02 * df["discount_rank_sess"].fillna(0).astype("float32")
    if "cheap_rank_sess" in df.columns:
        session_bonus = session_bonus + 0.01 * df["cheap_rank_sess"].fillna(0).astype("float32")
    if "rating_avg_rank_sess" in df.columns:
        session_bonus = session_bonus + 0.01 * df["rating_avg_rank_sess"].fillna(0).astype("float32")

    df["pred_final"] = (df["pred_final"] + df["repeat_bonus"] + session_bonus).astype("float32")
